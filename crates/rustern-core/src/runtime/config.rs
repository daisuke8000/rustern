...
impl CoreRunConfig {
    /// Minimal orchestration-test defaults; pair with [`crate::run_with_client`].
    #[doc(hidden)]
    pub fn for_test(follow: bool, root_token: CancellationToken) -> Self {
        Self::for_test_with_max_log_requests(follow, 5, root_token)
    }

    /// Like [`Self::for_test`] with a custom concurrent log-stream cap.
    #[doc(hidden)]
    pub fn for_test_with_max_log_requests(
        follow: bool,
